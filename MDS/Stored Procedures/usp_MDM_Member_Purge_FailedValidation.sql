/*
====================================================================================================
-- Create/Alter procedure usp_MDM_Member_Purge_FailedValidation
-- Author: Preeti Sinha
y
-- Import Types:
...
-- TEST:
	...
	EXEC dbo.usp_MDM_Member_Purge_FailedValidation @Entity='Credit_Card_Goals', @Model='Metric_Uploads'
	
	--DROP PROCEDURE dbo.usp_MDM_Member_Purge_FailedValidation
====================================================================================================
*/

CREATE PROCEDURE [dbo].[usp_MDM_Member_Purge_FailedValidation]
	@Entity VARCHAR(250),
	@Model VARCHAR(250),
	@ProcessID INT = -1
AS
	...
	SET @Job_ID = 7
	SET @Job_Shrt_Desc = 'usp_MDM_Member_Purge_FailedValidation'

	--******************************************** BEGIN PROCEDURE ********************************************

	SET @LogString = '**Begin purge of Failed Validation members in MDS entity ' + @Entity + '.**'
	...
	BEGIN CATCH
        ...
        SELECT @LogString = @LogString + 'Procedure: usp_MDM_Member_Purge_FailedValidation'
        INSERT  INTO dbo.TALTH_JOB_LOG
        VALUES  ( @Job_ID, @ProcessID, @Job_Shrt_Desc, @LogString, GETDATE() )
	...
	BEGIN TRY
		...
		FROM MDS.mdm.' + @EntityTable + ' 
		WHERE ValidationStatus_ID = 2'
		EXEC sp_executesql @Sql

		SET @RowCount = @@ROWCOUNT

		IF @RowCount > 0
		SET @LogString = 'Batch for ' + @Entity + ' created. BatchTag: ' + @BatchTag + '. Row Count: ' + CAST(@RowCount AS VARCHAR(8))
		INSERT  INTO dbo.TALTH_JOB_LOG
		VALUES  ( @Job_ID, @ProcessID, @Job_Shrt_Desc, @LogString, GETDATE() );
